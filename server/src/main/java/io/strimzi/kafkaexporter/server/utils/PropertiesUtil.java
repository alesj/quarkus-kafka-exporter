...
public class PropertiesUtil {
    ...
    private static final boolean debug = Boolean.getBoolean("kafka_exporter.debug");

    /**
     * Filter/strip prefixes from configuration properties.
     *
     * @param ip the injection point annotated with {@link InjectedProperties}
     * @return filtered/stripped properties
     */
    public static Properties properties(InjectionPoint ip) {
        InjectedProperties cp = ip.getAnnotated().getAnnotation(InjectedProperties.class);
        if (cp == null) {
            throw new IllegalArgumentException(
                ip.getMember() + " is not annotated with @InjectedProperties"
            );
        }
        ...
        if (prefixes.length == 0) {
            throw new IllegalArgumentException(
                "Annotation @InjectedProperties on " + ip.getMember() +
